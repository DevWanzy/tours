...
function HomePage() {
  ...
  useEffect(() => {
    fetch(`${c.backendUrl}/images/random`, { method: 'GET' }).then(
      async (re) => {
        if (re.ok) {
    ...
    fetch(`${c.backendUrl}/places/random`, { method: 'GET' }).then(
      async (re) => {
        if (re.ok) {
          let rnd = await re.json();
          setRandomPlace(rnd);
    ...
    <div className="homepage">
      <Search
        ...
        setPlace={(p) => {
          setShowSearch(false);
          selectDestination(0);
          fetch(`${c.backendUrl}/places/${p}`, { method: 'GET' }).then(
            async (re) => {
              if (re.ok) {
                let rnd = await re.json();
          ...
          <div id="refexbtndiv" style={{ display: 'flex' }}>
            <div
              ...
              onClick={() => {
                ...
                fetch(`${c.backendUrl}/places/random?prev=${randomPlace.rnd}`, {
                  method: 'GET',
                }).then(async (re) => {
                  if (re.ok) {
